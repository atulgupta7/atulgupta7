...
<!-- 3D isometric contribution calendar (generated by .github/workflows/metrics.yml -> commits github-metrics-isocalendar.svg to main) -->
<h3 align="center">🗓️ Contribution Calendar</h3>
<p align="center">
  <img width="92%" src="https://raw.githubusercontent.com/atulgupta7/atulgupta7/main/github-metrics-isocalendar.svg" alt="3D isometric contribution calendar" />
</p>
...
- The github-readme-stats summary card ("Total Commits") was REMOVED: it counts only PUBLIC commits
  (GitHub's commit-search index excludes private repos), so it undercounted private work. Streak + activity
  graph were kept because they read the contribution calendar, which DOES include (anonymized) private
  contributions when "Include private contributions on my profile" is enabled.
- "Most Used Languages" (top-langs) card was also intentionally removed (repos report language=null).
...
- 3D isometric calendar = lowlighter/metrics via .github/workflows/metrics.yml. REQUIRES a repo secret
  METRICS_TOKEN (classic PAT, scopeless for public data / add 'repo' for private). Commits
  github-metrics-isocalendar.svg to main on each run; README embeds it. Image 404s until the first successful run.
